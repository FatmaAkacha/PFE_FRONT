...
savedDoc: Document;
 ...
 selectedMagasinier: Magasinier = {} as Magasinier;
  ...
  ngOnInit(): void {
    ...
    sessionStorage.setItem('codeClasseDoc', 'FC')
    this.loadDoc();
    ...
    this.calculerTotal();
    console.log(this.factureProduits);
...
getDocumentClassesAndLoadNextCode() {
 ...
 getDocumentClassIdByLabel(label: string): number | null {
  ...
  calculerTotal() {
    this.totalHT = this.factureProduits.reduce((sum, p) => sum + p.prixTotal, 0);
  ...
  voirProduitsCommandes() {
    this.router.navigate(['/vente/produits-commandes'], {
      state: { produits: this.factureProduits.map(d => d.produit) }
    });
  }
  saveFactureAsDocument() {
    ...
    const document: Document = {
      ...
      libelle: 'Facture',  // Libelle du document
      code: '',  
    ...
    this.documentService.saveDocument(document).subscribe({
      next: (savedDoc) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Succès',
          detail: `Facture enregistré sous le code ${savedDoc.code}`
        });
      ...
      error: () => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erreur',
          detail: `Échec de l'enregistrement du document 'Facture'.`
        });
      }
    });
  }
  ...
  sauvegarderBonCommande() {
    console.log('Facture validé et sauvegardé');
  } 
